...
public class Straightener_V2 implements PlugIn {
	...
	static boolean secretSauce;
 	...
 	public void run(String arg) {
		...
		if (width<=1 || isMacro || stackSize>1) {
			...
			gd.addCheckbox("Retain Channels?", secretSauce);
			gd.showDialog();
		...
		if (processStack) {
			...
			if (secretSauce) {
				imp2 = straightenNORGB(imp, roi, width);
			} else {			
		...
		// 	imp2 = RGBStackMerge.mergeChannels(channels, false);
		// }
		for (int i = 0; i < imp2.getNChannels(); i++){
			imp2.setC(i+1);
			imp2.setDisplayRange(0,85.0);
			imp2.updateChannelAndDraw();
		}
	...
	public ImagePlus straightenNORGB(ImagePlus imp, Roi roi, int width) {
		ImageProcessor ip2;
		if (imp.getBitDepth()==24 && roi.getType()!=Roi.LINE)
			ip2 = straightenRGB(imp, width);
		else if (imp.isComposite() && ((CompositeImage)imp).getMode()==IJ.COMPOSITE) {
			if (roi.getType()==Roi.LINE)
				...
				ImagePlus imp2 = straightenCompositeNORGB(imp, width);
				return imp2;
			}
		} else if (roi.getType()==Roi.LINE)
			ip2 = rotateLine(imp, width);
	...
	ImagePlus straightenCompositeNORGB(ImagePlus imp, int width) {
		int n = imp.getNChannels();
		Roi roi = imp.getRoi();
		
		ImagePlus[] imgChannels = ChannelSplitter.split(imp);
		ImageProcessor[] channels = new ImageProcessor[n];
		
		for (int i = 0; i < n; i++) {
			Image img = imgChannels[i].getImage();
			ImagePlus imp2 = new ImagePlus("temp", new ColorProcessor(img));
			imp2.setRoi((Roi)roi.clone());
			ImageProcessor channel = straightenLine(imp2, width);
			channels[i] = channel;
		}
		ImagePlus[] imgChannelsStraight = new ImagePlus[n];

		for (int i = 0; i < n; i++) {
			imgChannelsStraight[i] = new ImagePlus("temp", channels[i]);
		}
		ImagePlus rtn = RGBStackMerge.mergeChannels(imgChannelsStraight, false);
		return rtn;
	}
